...
*
* Either (name and dataset) or (snapshots) must be provided.
*
* PARAMS:
* opts: {
*   name: string //the name of the snapshot to create (conditional)
*   dataset: string //name of the dataset to snapshot (conditional)
*   recursive: boolean //if true the -r option is added to the zfs command (optional)
*   snapshots: array/comma separated string //an array of snapshots to create in dataset@snapshot format (conditional)
* }
*
 */
...
function snapshot(opts, cb) {
    ...
    if (opts.dataset && opts.name) {
        params.push(opts.dataset + '@' + opts.name);
    }

    if (opts.snapshots) {
        var snaps = opts.snapshots;

        if (!Array.isArray(snaps)) {
            snaps = snaps.split(',');
        }

        params = params.concat(snaps);
    }
    ...
    spawnzfs(params, function (err, child) {
        ...
        if (opts.verbose) {
            child.stdout.on('data', function (data) {
                receiveStream.emit('verbose', data.toString());
            });
        }
